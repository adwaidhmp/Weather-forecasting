...
async function checkWeather(city) {
    ...
    const response2 =await fetch(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${latti}&lon=${longi}&appid=${apikey}`)
...
if (hour >= 18 || hour < 6) {
    ...
    document.bg.style.background = "linear-gradient(to bottom, black, grey)";
} else {
    // Daytime (you can set a different background if you want)
    document.bg.style.background = "linear-gradient(to bottom, skyblue, lightyellow)";
}
